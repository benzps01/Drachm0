...
export default function EditTransactionScreen({ navigation, route }) {
  ...
  const [particulars, setParticulars] = useState(transaction.particulars);
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const currentCategoryIsValid = categoryData.some(cat => cat.id.toString() === categoryId);
        
        if (!currentCategoryIsValid && categoryData.length > 0) {
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      await db.updateTransaction(transaction.id, formatDate(date), type === 'expense' ? mode : 'N/A', particulars, numAmount, type, parseInt(categoryId));
      Alert.alert('Success', 'Transaction updated successfully', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (error) {
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <ScrollView>
        <Card style={[styles.card, { backgroundColor: theme.colors.card }]}>
          <Card.Content>
            <Title style={[styles.title, { color: theme.colors.text }]}>Edit Transaction</Title>
            ...
            <View style={styles.actionButtons}>
              <Button mode='outlined' onPress={() => navigation.goBack()} style={styles.actionButton}>Cancel</Button>
              <Button mode='contained' onPress={handleUpdate} loading={loading} style={styles.actionButton}>Update</Button>
            </View>
          </Card.Content>
        </Card>
      </ScrollView>
